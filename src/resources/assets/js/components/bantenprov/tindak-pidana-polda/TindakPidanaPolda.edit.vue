...
export default {
  mounted() {
    axios.get('api/tindak-pidana-polda/' + this.$route.params.id + '/edit')
      .then(response => {
        if (response.data.status == true) {
          this.model.label = response.data.tindak_pidana_polda.label;
          this.model.old_label = response.data.tindak_pidana_polda.label;
          this.model.description = response.data.tindak_pidana_polda.description;
        } else {
          alert('Failed');
        }
      })
      .catch(function(response) {
        alert('Break');
        window.location.href = '#/admin/tindak-pidana-polda';
      });
  },
  data() {
    return {
  ...
  methods: {
    onSubmit: function() {
      ...
      if (this.state.$invalid) {
        ...
        axios.put('api/tindak-pidana-polda/' + this.$route.params.id, {
    ...
    back() {
      window.location = '#/admin/tindak-pidana-polda';
    }
  }
}
</script>
